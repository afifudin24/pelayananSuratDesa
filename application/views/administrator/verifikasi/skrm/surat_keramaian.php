<div class="main-content container-fluid">
	<div class="page-title">
		<h4>Verifikasi Surat Keramaian</h4>
	</div>
								...
								<tbody>
									...
									foreach ($datas as $data) { ?>
										<tr>
											...
											<td>
												<a href="<?= base_url('preview-skrm/' . $data->id) ?>"><span
											...
											<td>
												<?php if ($this->session->userdata('role_id') == 1) { ?>
													...
													<?php if ($data->status == 'Menunggu Verifikasi') { ?>
														<div class="btn-group">
															<button class="btn btn-info btn-sm" data-toggle="modal"
																data-target="#tambahKomentar<?= $data->id ?>">Komentar</button>
															<form action="<?= base_url('update-status-skrm') ?>" method="post"
													...
													<?php } else if ($data->status == 'Diterima') { ?>
																<div class="btn-group">
																	<a href="<?= base_url('cetak-surat-keramaian/' . $data->id . '?nomor=' . $data->nomor_surat) ?>"
																		class="btn btn-primary btn-sm" target="blank_"><i
														...
														<div class="modal-dialog">
															<div class="modal-content">
																<div class="modal-header bg-warning">
																	<h5 class="modal-title" id="exampleModalLabel">Konfirmasin
																		Hapus Data</h5>
																...
																<div class="modal-body">
																	<div class="alert alert-danger" role="alert">
																		<h6 class="text-center">Apakah Anda yakin menghapus data
																			surat ini ?</h6>
																		<small
																			class="text-cen"><?= $data->nomor_surat ?></small>
																	</div>
																</div>
																<div class="modal-footer">
																	<button type="button" class="btn btn-secondary btn-sm"
																		data-dismiss="modal">Close</button>
																	<a href="<?= base_url('hapus-surat-tidak-mampu/' . $data->id) ?>"
																...
																<div class="btn-group">
																	<form action="<?= base_url('update-status-skrm') ?>" method="post"
																		...
																		<input type="hidden" name="status" value="Diterima"
																			class="form-control" readonly>
																		<button
																			class="btn btn-<?= $data->status == 'Diterima' ? 'success' : 'light' ?> btn-sm"
																			type="submit">Terima</button>
																	</form>
																	<form action="<?= base_url('update-status-skrm') ?>" method="post"
																		enctype="multipart/form-data" readonly>
																		<input type="hidden" name="id" value="<?= $data->id ?>"
																			class="form-control" readonly>
																		<input type="hidden" name="id_warga" value="<?= $data->id_warga ?>"
																			class="form-control" readonly>
																		<input type="hidden" name="status" value="Ditolak"
																			class="form-control" readonly>
																		<button
																			class="btn btn-<?= $data->status == 'Ditolak' ? 'danger' : 'light' ?> btn-sm"
														...
														<div class="modal-content">
															...
															<div class="modal-body">
																<div class="row">
																	<div class="col-lg-12">
																		<form
																			action="<?= base_url('komentar-skrm/') . $data->id ?>"
																			method="post" enctype="multipart/form-data">
																			<div class="form-group">
																				<label for="">Komentar</label>
																				<textarea name="komentar"
																					class="form-control" cols="30"
																					rows="10"><?= $data->komentar ?></textarea>
																			</div>
																			<div class="form-group">
																				<button class="btn btn-success btn-sm"
