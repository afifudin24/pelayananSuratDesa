...
class M_notifikasi extends CI_Model
{
    ...
    public function nSkrm()
    {
        $query = $this->db->query("SELECT COUNT(*) AS jumlah FROM surat_keramaian WHERE notifikasi = '1' ");
        return $query->row_array();
    }
    ...
    public function updateSkrm()
    {
        $this->db->set('notifikasi', '0');
        $this->db->update('surat_keramaian');
    }
    ...
    public function aSkrm()
    {
        if ($this->session->userdata('role_id') == 1) {
            $query = $this->db->query("SELECT COUNT(*) AS jumlah FROM surat_keramaian WHERE status = 'Menunggu Verifikasi'");
            return $query->row_array();
        } else if ($this->session->userdata('role_id') == 2) {
            $query = $this->db->query("SELECT COUNT(*) AS jumlah FROM surat_keramaian WHERE status = 'Terverifikasi'");
            return $query->row_array();
        }
    }
}
